 ...
 * \ingroup geo
 *
 * Fix 3-edge poles in meshes by collapsing them.
...
/**
 * Check if mesh has any 3-edge poles that would be affected by the fix operation.
 */
static bool mesh_has_3_edge_poles(BMesh *bm)
{
  BMVert *v;
  BMIter iter;
...
std::optional<Mesh *> mesh_fix_poles(const Mesh &mesh, const int smooth_iterations)
{
  ...
  /* Early exit if no 3-edge poles exist. */
  if (!mesh_has_3_edge_poles(bm)) {
    BM_mesh_free(bm);
    return std::nullopt;
  }
  ...
  /* Update mesh topology after filling gaps. */
  ...
  /* Convert back to Mesh - use _for_eval_nomain for geometry nodes context. */
